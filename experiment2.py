...
def traverse_less_than_30(path):
    ...
    for root, directories, files in os.walk(path):
        for file in files:
            if file.startswith("ss") and file.endswith("json") and float(file.split(":")[3].split(".json")[0].split("ms")[0])<30: # for filess where RTT is less than 30 
...
def traverse_greater_than_30(path):
    ...
    for root, directories, files in os.walk(path):
        for file in files:
            if file.startswith("ss") and file.endswith("json") and float(file.split(":")[3].split(".json")[0].split("ms")[0])>=30: # for filess where RTT is greater than 30 
                fileList.append(os.path.join(root,file))
                c+=1
    print(f"Total files: {c}")
    return fileList
